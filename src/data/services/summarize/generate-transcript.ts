...
const getTranscriptSegments = async (info: any): Promise<YouTubeTranscriptSegment[]> => {
...
export const generateTranscript = async (identifier: string): Promise<TranscriptData> => {
  try {
    ...
    const { title, videoId, thumbnailUrl } = extractBasicInfo(info);
    const segments = await getTranscriptSegments(info);
